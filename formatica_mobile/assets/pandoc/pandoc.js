...
console.log('[PandocJS] Fetching pandoc.wasm...');
let wasmResponse;
try {
  // Try with absolute path resolved from import.meta.url
  const wasmUrl = new URL("./pandoc.wasm", import.meta.url).toString();
  wasmResponse = await fetch(wasmUrl);
} catch (e) {
  console.warn('[PandocJS] fetch(URL) failed, trying relative ./pandoc.wasm...', e);
  wasmResponse = await fetch("./pandoc.wasm");
}

if (!wasmResponse.ok) {
  console.error('[PandocJS] Failed to fetch pandoc.wasm:', wasmResponse.statusText);
  throw new Error(`HTTP error fetching WASM: ${wasmResponse.status}`);
}
const wasmBytes = await wasmResponse.arrayBuffer();
console.log(`[PandocJS] Loaded ${wasmBytes.byteLength} bytes of WASM`);

if (!wasmBytes.byteLength) {
  throw new Error("pandoc.wasm could not be loaded");
}

console.log('[PandocJS] Instantiating WebAssembly...');
const { instance } = await WebAssembly.instantiate(wasmBytes, {
  wasi_snapshot_preview1: wasi.wasiImport,
});
console.log('[PandocJS] WebAssembly instantiated');

wasi.initialize(instance);
instance.exports.__wasm_call_ctors();
console.log('[PandocJS] Runtime initialized');
